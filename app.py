...
st.caption("Processing 2,000+ Listed NSE Stocks using Live Bulk EOD File Data (100% List-Free)")

# 1. TRULY DYNAMIC LOG-FREE DATA CONTROLLER
@st.cache_data(ttl=14400)
def download_complete_nse_bhavcopy():
    # Pull directly from the up-to-date repository directory
    ...
    # Formats every row straight from the web source file cleanly with no internal fallback references
...
st.info("📡 Fetching the broad exchange data block... Parsing active tickers...")

try:
    # Trigger the 100% automated list-free scraper function
    UNRESTRICTED_NSE_POOL = download_complete_nse_bhavcopy()
    
    screened_results = []
    # Process the live downloaded database row-by-row smoothly
    ...
    st.error("⚠️ Connection Latency: The external stock database is temporarily busy. Please tap the button below to re-sync.")
    if st.button("🔄 Re-try Data Sync"):
        st.rerun()
